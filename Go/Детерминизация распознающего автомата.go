...
func Push(stack *([](*sort.IntSlice)), length *int, cell *sort.IntSlice) {
	(*stack)[*length] = cell
	*length++
}

func Pop(stack *([](*sort.IntSlice)), length *int) *sort.IntSlice {
	*length--
	return (*stack)[*length]
}

func DFS(list [][]elem, q int, c *sort.IntSlice, table *(map [int] int)) {
	_, flag := (*table)[q]
	if !flag {
		(*table)[q] = q
		(*c) = append(*c, q)
...
func Det(X []string, list [][]elem, circls []int, q int) {
	...
	arr_state, delta, F := make([]*sort.IntSlice, 1), make([][]*sort.IntSlice, 1), make([]*sort.IntSlice, 0)
	...
	//InitStack
	stack, length := make([](*sort.IntSlice), 1), 0
	Push(&stack, &length, q0)
	...
	for (length > 0) {
		...
		for i := 0; i < len(X); i++ {//For everyone element from X
			arr_u := make([]int, 0)
			for j := 0; j < len(*z); j++ {
			...
			z1 := Closure(list, arr_u) // z1 <- Closure(delta, Udelta(u, a)
...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		var flag bool
		var u int
		input.Scanf("%d %d %s", &u, &v.vertex, &v.label)
